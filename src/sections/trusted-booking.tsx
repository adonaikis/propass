...
function ImageTile({
  ...
  return (
    <div className={`relative overflow-hidden rounded-[10px] bg-zinc-200 ${className}`}>
      <Image
        src={src}
        alt={alt}
        fill
...
function CommunityCard({ className = "" }: { className?: string }) {
  return (
    ...
    >
      <div className="absolute -right-9 top-3 size-28 rounded-full border border-white/15" />
      <div className="absolute right-8 top-16 size-14 rounded-full border border-white/10" />
...
export default function TrustedBooking() {
  return (
    <section className="bg-white px-4 py-14 text-black md:py-16">
      <div className="mx-auto max-w-[1180px]">
        ...
        <h2 className="mx-auto mt-6 max-w-[800px] text-center font-heading text-[34px] font-medium leading-[0.98] tracking-normal sm:text-[40px] md:text-[42px] min-[1180px]:text-[56px]">
          <span>
            Séjours sûrs, réservation fluide
            <span className="mx-3 inline-block h-[2px] w-14 translate-y-[-0.18em] bg-black md:w-20" />
          </span>
          <span className="block">Explorez maintenant !</span>
        </h2>

        <p className="mx-auto mt-6 max-w-[520px] text-center text-xs font-medium leading-5 text-zinc-700">
          Découvrez les meilleurs hôtels près de vous en un geste, avec une réservation
        ...
        <div className="mt-9 flex flex-wrap items-center justify-center gap-x-8 gap-y-4 md:flex-nowrap md:justify-between md:gap-x-5">
        ...
        <div className="mt-9 grid gap-4 sm:grid-cols-2 md:hidden">
          {galleryImages.slice(0, 3).map((image) => (
            <ImageTile key={image.src} src={image.src} alt={image.alt} className="h-[210px]" />
          ))}
        ...
        <div className="mt-9 hidden items-start justify-between gap-3 md:flex min-[1180px]:gap-5">
          {galleryImages.slice(0, 3).map((image) => (
            <ImageTile key={image.src} src={image.src} alt={image.alt} className={image.className} />
          ))}
