...
import { ArrowLeft, ArrowRight, ArrowUpRight, Heart, Search } from "lucide-react";
...
export default function BestHotels() {
  return (
    <section className="bg-[#f7f7f7] px-4 py-14 text-black md:py-16">
      <div className="mx-auto max-w-[1180px]">
        <div className="flex flex-col gap-6 min-[860px]:flex-row min-[860px]:items-center min-[860px]:justify-between">
          <div className="flex items-center gap-4">
            <span className="rounded-full border border-zinc-300 bg-white px-3 py-1 text-[11px] font-medium">
              Installations
            </span>
            <h2 className="whitespace-nowrap font-heading text-[22px] font-semibold leading-none md:text-[24px]">
          ...
          <div className="flex items-center gap-3">
            <label className="flex h-10 w-full min-w-0 items-center gap-3 rounded-full bg-white px-5 text-xs text-zinc-500 shadow-sm min-[860px]:w-[300px] lg:w-[430px]">
              ...
              <input
                ...
                type="search"
              />
              <Search className="size-4 shrink-0 text-black" strokeWidth={2.2} aria-hidden="true" />
            </label>
        ...
        <div className="mt-10 grid gap-3 sm:grid-cols-2 md:grid-cols-4">
          {hotels.map((hotel) => (
            ...
            >
              ...
              >
                <Heart className="size-4 fill-current" strokeWidth={2} aria-hidden="true" />
              </button>
              ...
              <div className="absolute inset-x-4 bottom-4 rounded-[14px] bg-white/25 p-4 text-white backdrop-blur-md">
                ...
                <div className="mt-3 flex items-end justify-between gap-4">
                  <h3 className="text-[18px] font-medium leading-none">{hotel.name}</h3>
                  <button
                    type="button"
                  ...
                  >
                    <ArrowUpRight className="size-5" strokeWidth={2.3} aria-hidden="true" />
                  </button>
                </div>
              </div>
            </article>
        ...
        <div className="mt-10 flex flex-col gap-8 min-[860px]:flex-row min-[860px]:items-start min-[860px]:justify-between">
          <div className="flex items-center gap-3">
            <button
              type="button"
          ...
          <p className="max-w-[360px] text-right text-xs font-medium leading-5 text-zinc-700 max-[859px]:text-left">
            Réservez votre séjour pour des expériences personnalisées, des équipements
            luxueux et une escapade relaxante vers des souvenirs inoubliables.
          </p>
        </div>
      </div>
    </section>
